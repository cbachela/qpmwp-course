    ...
    def add_linear(self,
        ...
        if G is None:
            if g_values is None:
                ...
                if name is not None:
                    G.index = [name]
        # If the matrix G is not provided, it is created from the vector "g_values". The vector "g_values" is just a vector (pd.Series), so could only be used for one constraint.
        # If the vector "g_values" do not match all column ID's of the G matrix, the missing columns are filled with 0.
        # If the matrix G is generated by the vector "g_values", the index of the G matrix is set to the name of the constraint (if provided).
